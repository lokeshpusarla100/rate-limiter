...
## 🚀 Epic 1.6: Engineering Hardening (The 12 Fixes) [IN PROGRESS]
**Goal**: Address all critical and priority gaps in the core domain and contract.

### Logic & Security Fixes
*   **[Fix 6] Domain-Driven Logic**: [DONE] Move consumption logic from `DefaultRateLimiter` into `TokenBucket.tryConsume()`.
*   **[Fix 2] Plan Security**: [DONE] Implement `MissingPlanPolicy` (FAIL_FAST, SKIP, ALLOW) to prevent silent bypasses.
*   **[Fix 4] Semantic Clarity**: [DONE] Update `RateLimitResult` to explicitly return the *minimum* tokens across chained limits.
*   **[Fix 8] Key Standardization**: [DONE] Implement `RateLimitKey` builder to prevent collisions and undefined formats.

### Observability & Resilience
*   **[Fix 7] Event Hooks**: [DONE] Implement `RateLimitEventListener` port for Micrometer/Logging integration.
*   **[Fix 3] Circuit Breaker**: [PENDING] Prepare the `Repository` wrapper for Resilience4j integration.
*   **[Fix 5] Timeout Contract**: [DONE] Explicitly define and document the 100ms SLA in the Repository port.

### Documentation & Safety (ADRs)
*   **[Fix 9] Precision Audit**: [DONE] Add Javadoc and tests for floating-point bounds.
*   **[Fix 10] Clock Skew**: [DONE] Document backwards-clock behavior in Javadoc.
*   **[Fix 11] Overhead Disclosure**: [DONE] Document network/payload trade-offs.
*   **[Fix 12] Thread Safety**: [DONE] Explicitly document the concurrency contract for all Ports.

### Compatibility
*   **[Fix 1] Critical Compatibility**: [PENDING] Implement the `JavaLuaCompatibilityTest` using Testcontainers.
